...
export default function Home() {
  ...
  return (
    <main className="fixed inset-0 bg-[#050505] flex items-center justify-center overflow-hidden" style={{ color: themeColor } as CSSProperties}>
      ...
      {/* NOTIFICATION TOAST */}
      {toast && (
        <div className="fixed top-4 left-1/2 -translate-x-1/2 z-[100] w-[90%] max-w-sm bg-black border p-3 shadow-[0_0_20px_rgba(0,0,0,0.8)] animate-in slide-in-from-top-full duration-300" style={{ borderColor: themeColor }}>
          <p className="text-[10px] font-black uppercase tracking-widest opacity-50">Incoming_Transmission</p>
          <p className="text-xs font-bold text-white mt-1">@{toast.from}: <span className="font-normal opacity-80">{toast.msg.substring(0, 30)}...</span></p>
        </div>
      )}
      ...
      {!isLoggedIn ? (
        ...
        <div className="w-full h-full md:h-[92vh] md:max-w-6xl grid grid-cols-1 md:grid-cols-[260px_1fr] md:border border-zinc-900 bg-black md:rounded-sm overflow-hidden shadow-2xl">
          ...
          <div className={`${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 fixed md:relative z-50 w-[85%] md:w-full h-full border-r border-zinc-900 bg-[#080808] p-6 flex flex-col justify-between transition-transform duration-300 ease-in-out`}>
            <div className="overflow-y-auto scrollbar-hide">
              ...
              <div className="mb-8">
                ...
                <input className="w-full bg-[#111] border border-zinc-800 p-3 text-sm outline-none text-white" placeholder="FIND_HANDLE..." value={userSearch} onChange={(e) => handleUserSearch(e.target.value)} />
                {searchResults.length > 0 && (
                  <div className="mt-2 border border-zinc-800 bg-black max-h-40 overflow-y-auto shadow-xl">
                    {searchResults.map(u => (
                      <button key={u.username} onClick={() => startPrivateChat(u.username)} className="w-full text-left p-3 text-[11px] hover:bg-zinc-900 border-b border-zinc-900 last:border-0 uppercase font-bold">{u.username}</button>
                    ))}
                  </div>
                )}
              </div>
          ...
          <div className="flex flex-col h-full bg-[#0a0a0a] relative overflow-hidden">
                  ...
                  return (
                    <div key={msg._id || index} className={`flex flex-col ${isMe ? "items-end" : "items-start"}`}>
                      <div className="max-w-[92%] md:max-w-[75%] group relative">
                        <span className={`text-[9px] text-zinc-600 uppercase mb-1 block font-bold ${isMe ? "text-right" : "text-left"}`}>{msg.sender}</span>
                        <div className="p-3 md:p-4 rounded-sm relative bg-white/5 border-zinc-700 text-zinc-300" style={{ borderRightWidth: isMe ? '2px' : '0px', borderLeftWidth: isMe ? '0px' : '2px', borderColor: isMe ? themeColor : '#3f3f46' } as CSSProperties}>
            ...
            {!showShop && (
              <div className="p-3 md:p-4 bg-black border-t border-zinc-900">
                {showGifs && (
                  <div className="mb-4 bg-[#0d0d0d] border border-zinc-800 p-4 rounded-sm shadow-2xl">
                    <input className="w-full bg-black border border-zinc-800 p-3 text-sm outline-none transition-all mb-4" style={{ color: themeColor, borderColor: themeColor } as CSSProperties} placeholder="Search Giphy..." value={gifSearch} onChange={(e) => setGifSearch(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && searchGifs()} />
                    <div className="grid grid-cols-3 gap-2 h-40 overflow-y-auto">
                        {gifs.map((g) => ( <img key={g.id} src={g.images.fixed_height_small.url} className="w-full h-24 object-cover" onClick={() => sendGif(g.images.fixed_height.url)} /> ))}
                    </div>
                  </div>
                )}
                <form onSubmit={sendMessage} className="flex gap-2 items-center">
                  ...
                  <input type="text" className="flex-1 bg-[#0f0f0f] border border-zinc-800 p-3 text-base focus:outline-none text-white" onFocus={(e) => e.target.style.borderColor = themeColor} onBlur={(e) => e.target.style.borderColor = '#27272a'} placeholder="Type..." value={message} onChange={(e) => setMessage(e.target.value)} />
