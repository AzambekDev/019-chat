        ...
        <div className="w-full h-full md:h-[92vh] md:max-w-6xl grid grid-cols-1 md:grid-cols-[260px_1fr] md:border border-zinc-900 bg-black md:rounded-sm overflow-hidden">
          ...
          <div className={`${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 fixed md:relative z-50 w-[85%] md:w-full h-full border-r border-zinc-900 bg-[#080808] p-6 flex flex-col justify-between transition-transform duration-300 ease-in-out`}>
            <div className="overflow-y-auto scrollbar-hide">
              ...
              {/* Channels & DMs logic remains same as your original */}
              <div className="space-y-1 mb-6">
                <p className="text-[9px] text-zinc-600 mb-2 uppercase tracking-widest font-bold">Channels</p>
                {CHANNELS.map((ch) => (
                  <button key={ch} onClick={() => { setShowShop(false); setCurrentRoom(ch); setIsSidebarOpen(false); }} className={`w-full text-left text-[11px] p-3 uppercase font-bold flex justify-between items-center ${currentRoom === ch && !showShop ? 'bg-white/5 border-l-2' : 'opacity-40'}`} style={{ borderColor: currentRoom === ch && !showShop ? themeColor : 'transparent' }}>
                    <span># {ch}</span>
                    {notifications[ch] > 0 && <span className="bg-red-600 text-white text-[10px] px-2 rounded-full">{notifications[ch]}</span>}
                  </button>
                ))}
              </div>
              <button onClick={() => {setShowShop(!showShop); setIsSidebarOpen(false);}} className="mt-4 w-full text-left text-[10px] font-black tracking-widest uppercase" style={{ color: showShop ? '#fff' : themeColor }}>{showShop ? '[ CLOSE_TERMINAL ]' : '[ ACCESS_STORE ]'}</button>
            </div>
          ...
          <div className="flex flex-col h-full bg-[#0a0a0a] relative overflow-hidden">
            <div className="p-4 border-b border-zinc-900 flex justify-between items-center bg-black/50 h-16 backdrop-blur-md">
              <div className="flex flex-col">
                <span className="text-[10px] uppercase font-bold opacity-30 tracking-[0.2em]">{currentRoom.toUpperCase()}</span>
            ...
            {showShop ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {THEMES.map((t) => (
                    <div key={t.id} className="border border-zinc-800 p-6 bg-black flex flex-col justify-between hover:border-zinc-500 transition-all">
                      <div><h3 className="font-black text-lg italic uppercase" style={{ color: t.color }}>{t.name}</h3><p className="text-[10px] text-zinc-500 mt-1">Cost: {t.cost.toFixed(2)} ⌬</p></div>
                      {unlockedThemes.includes(t.id) ? (
                        ...
                        <button onClick={() => handlePurchase(t.id, t.cost)} disabled={coins < t.cost} className="mt-6 text-[10px] font-black py-3 border border-yellow-600 text-yellow-600 uppercase transition-all">Buy</button>
                      )}
                    </div>
                  ))}
                </div>
              ...
              <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-6 scrollbar-hide font-mono">
                {chat.map((msg, index) => {
                  ...
                  const isRead = msg.seenBy && msg.seenBy.length > 1; // Simplistic seen logic for group/DM
                  return (
                    <div key={msg._id || index} className={`flex flex-col ${isMe ? "items-end" : "items-start"} animate-in fade-in slide-in-from-bottom-1 duration-300`}>
                      <div className="max-w-[92%] md:max-w-[75%] group relative">
                        ...
                        <div className={`p-3 md:p-4 rounded-sm relative bg-white/5 border-zinc-700 text-zinc-300 shadow-sm ${msg.isVanishing ? 'border-orange-900 italic' : ''}`} 
                             style={{ borderRightWidth: isMe ? '2px' : '0px', borderLeftWidth: isMe ? '0px' : '2px', borderColor: isMe ? (msg.isVanishing ? '#7c2d12' : themeColor) : '#3f3f46' }}>
              ...
              <div className="p-3 md:p-4 bg-black border-t border-zinc-900 pb-safe">
                <div className="flex gap-2 mb-2">
                  <button onClick={() => { setEncryptEnabled(!encryptEnabled); playSystemBeep(encryptEnabled ? 440 : 880, 0.05); }}
                    ...
                    {encryptEnabled ? "SECURE_LINK_ACTIVE" : "PLAINTEXT"}
                  </button>
                  <button onClick={() => { setIsVanishing(!isVanishing); playSystemBeep(isVanishing ? 300 : 1200, 0.05); }}
                          className={`text-[8px] font-black px-2 py-1 border transition-all ${isVanishing ? "bg-orange-600 text-white border-orange-600 shadow-[0_0_10px_rgba(234,88,12,0.5)]" : "border-zinc-800 opacity-40"}`}>
                    {isVanishing ? "VANISHING_ENABLED" : "STATIC_STORAGE"}
                  </button>
                </div>
                <form onSubmit={sendMessage} className="flex gap-2 items-center">
                  <button type="button" onClick={() => setShowGifs(!showGifs)} className={`px-4 py-3 text-[10px] font-black border uppercase transition-all ${showGifs ? "text-black" : "text-zinc-500"}`} style={{ backgroundColor: showGifs ? themeColor : 'transparent', borderColor: showGifs ? themeColor : '#27272a' }}>GIF</button>
                  <input type="text" className="flex-1 bg-[#0f0f0f] border border-zinc-800 p-3 text-sm text-white focus:outline-none" placeholder={isVanishing ? "Destructible transmission..." : "Awaiting input..." } value={message} onChange={(e) => setMessage(e.target.value)} />
                  <button type="submit" className="px-8 py-3 font-black text-[10px] uppercase text-black" style={{ backgroundColor: themeColor }}>SEND</button>
                </form>
              </div>
            )}
          </div>
